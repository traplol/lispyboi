  ...
  (if (fixnump number)
      (if (= number 0)
          ...
          (let ((digits)
            ...
            (while (/= number 0)
                   (when (< number 0)
                     ;; This needs to be inside the loop because (abs +most-negative-fixnum+)
                     ;; returns +most-negative-fixnum+. This is a side-effect of having
                     ;; no INTEGER-type object to promote to.
